...
<!-- /header--> 
<!-- banner-->
<div class="subBanner"> <img src="images/detailsNav.jpg" /> </div>
<!-- /banner--> 
<!-- notice-->
<!-- /notice--> 
<!-- mainbody-->
<div class="subBody">
	...
	<div class="OneOfTwo">
		...
		<div class="listConts">
			...
			//检测文档正确性
			$row = $dosql->GetOne("SELECT * FROM pmw_infolist  WHERE id=$id");
			if(is_array($row))
			{
				//增加一次点击量
				$dosql->ExecNoneQuery("UPDATE pmw_infolist SET hits=hits+1 WHERE id=$id");
			?>
			<!-- 标题区域开始 -->
			...
			<div class="preNext">
				...
				//获取上一篇信息
				$r = $dosql->GetOne("SELECT * FROM pmw_infolist WHERE classid=".$row['classid']." AND orderid<".$row['orderid']." AND delstate='' AND checkinfo=true ORDER BY orderid DESC");
				if($r < 1)
				{
				...
				{
					if($cfg_isreurl != 'Y')
						$gourl = 'solutionshow.php?cid='.$r['classid'].'&id='.$r['id'];
					else
						$gourl = 'solutionshow-'.$r['classid'].'-'.$r['id'].'-1.html';

					echo '<li>上一篇：<a href="'.$gourl.'">'.$r['title'].'</a></li>';
				}
				
				//获取下一篇信息
				$r = $dosql->GetOne("SELECT * FROM pmw_infolist WHERE classid=".$row['classid']." AND orderid>".$row['orderid']." AND delstate='' AND checkinfo=true ORDER BY orderid ASC");
				if($r < 1)
				{
